    ...
    <Link href={`/content/${contentId}`}>
      <div className="flex items-center gap-4 p-3 rounded-lg hover:bg-[#191f31]/50 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[#86948a] truncate">
            {submission.description}
          </p>
        </div>
        <span
          className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono uppercase tracking-widest ${statusColors[submission.status] || statusColors[0]}`}
        >
  ...
  return (
    <div className="glass-card rounded-xl p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-heading text-lg font-bold text-[#dce1fb]">
          Recent Submissions
        </h3>
        <Link
          href="/content"
          className="text-xs font-mono text-[#38BDF8] uppercase tracking-widest hover:text-[#7bd0ff]"
        >
          View All →
        </Link>
      </div>
      ...
      <div className="space-y-2">
        {isLoading ? (
          ...
          <p className="text-sm text-[#86948a] py-4 text-center">
          ...
          contentIds
            .slice(-3)
            .reverse()
            .map((id) => <ContentRow key={Number(id)} contentId={Number(id)} />)
        )}
      </div>
    </div>
  );
