...
export function XPBreakdownChart({ profile }: XPBreakdownChartProps) {
  const attendance =
    Number(profile.matchesAttended) * XP_PER_ACTION.MATCH_ATTENDANCE;
  const governance = Number(profile.votesParticipated) * XP_PER_ACTION.VOTE;
  const content =
    Number(profile.contentSubmitted) * XP_PER_ACTION.CONTENT_APPROVED;
      ...
      <div className="flex items-center gap-6">
        <div className="relative w-40 h-40">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="font-mono text-2xl font-bold text-[#dce1fb]">
              {Number(profile.totalXP)}
            </span>
            <span className="text-[10px] font-mono uppercase tracking-widest text-[#86948a]">
              Total XP
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-3">
          {[
            {
              label: "Attendance",
              pct: total > 0 ? Math.round((attendance / total) * 100) : 33,
              color: "#4edea3",
            },
            {
              label: "Governance",
              pct: total > 0 ? Math.round((governance / total) * 100) : 33,
              color: "#38BDF8",
            },
            {
              label: "Content",
              pct: total > 0 ? Math.round((content / total) * 100) : 34,
              color: "#F59E0B",
            },
          ].map((item) => (
            <div key={item.label} className="flex items-center gap-3">
              ...
              />
              <span className="text-sm text-[#bbcabf] flex-1">
                {item.label}
              </span>
              <span className="font-mono text-sm text-[#dce1fb]">
                {item.pct}%
              </span>
            </div>
          ))}
