      ...
      <div className="glass-card rounded-xl p-5 hover:bg-[rgba(46,52,71,0.5)] transition-colors cursor-pointer">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono uppercase tracking-widest ${stateColors[proposal.state] || stateColors[0]}`}
            >
          ...
          {hasVoted !== undefined && (
            <span
              className={`text-[10px] font-mono uppercase tracking-widest ${hasVoted ? "text-[#4edea3]" : "text-[#F59E0B]"}`}
            >
        ...
        <p className="text-sm text-[#bbcabf] line-clamp-2 mb-3">
        ...
        <div className="flex gap-2 mt-3 flex-wrap">
          {proposal.options.slice(0, 4).map((opt, i) => (
            <span
              key={i}
              className="px-2 py-1 rounded bg-[#191f31] text-[10px] font-mono text-[#bbcabf]"
            >
              {opt}
            </span>
          ))}
    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-sm text-[#86948a] mt-2">
          Be the first to propose something for the PSL community!
        </p>
      </div>
    );
  }
