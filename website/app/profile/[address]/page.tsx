...
import { formatAddress, formatXP, getTierName, getTierColor, formatFAN, getNextTierThreshold } from '@/lib/utils';
import { TIER_BG_COLORS, WIRESCAN_URL } from '@/lib/constants';
import { Skeleton } from '@/components/ui/skeleton';
import { ExternalLink, Copy, Landmark, Film, Vote } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-[#020617]">
        ...
        <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <p className="text-lg text-[#86948a]">This address is not a registered CricketChain fan.</p>
          <p className="text-sm text-[#86948a] mt-2">{formatAddress(address)}</p>
        </main>
        <Footer />
      </div>
    );
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="font-heading text-2xl font-bold text-[#dce1fb]">{profile.username}</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="glass-card rounded-xl p-4 text-center">
            <p className="font-mono text-2xl font-bold text-[#4edea3]">{formatXP(xp)}</p>
            <p className="text-[10px] font-mono uppercase tracking-widest text-[#86948a]">Total XP</p>
          </div>
          <div className="glass-card rounded-xl p-4 text-center">
            <p className="font-mono text-2xl font-bold text-[#dce1fb]">{Number(profile.matchesAttended)}</p>
            <p className="text-[10px] font-mono uppercase tracking-widest text-[#86948a]">Matches</p>
          </div>
          <div className="glass-card rounded-xl p-4 text-center">
            <p className="font-mono text-2xl font-bold text-[#dce1fb]">{Number(profile.contentSubmitted)}</p>
            <p className="text-[10px] font-mono uppercase tracking-widest text-[#86948a]">Content</p>
          </div>
          <div className="glass-card rounded-xl p-4 text-center">
            <p className="font-mono text-2xl font-bold text-[#dce1fb]">{Number(profile.votesParticipated)}</p>
            <p className="text-[10px] font-mono uppercase tracking-widest text-[#86948a]">Votes</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="glass-card rounded-xl p-6">
            <p className="text-[10px] font-mono uppercase tracking-widest text-[#86948a] mb-2">FAN Token Balance</p>
            <p className="font-mono text-3xl font-bold text-[#F59E0B]">
              {balance !== undefined ? formatFAN(balance) : '—'}
            </p>
          </div>
          <div className="glass-card rounded-xl p-6">
            <p className="text-[10px] font-mono uppercase tracking-widest text-[#86948a] mb-2">Next Tier</p>
            ...
            <div className="h-2 bg-[#191f31] rounded-full overflow-hidden mt-2">
              <div
                ...
                style={{ width: `${Math.min((xp / nextThreshold) * 100, 100)}%` }}
              />
            </div>
          </div>
        </div>
        ...
        <p className="text-xs text-[#86948a] text-center">
          Fan since {new Date(Number(profile.registeredAt) * 1000).toLocaleDateString('en-US', {
            month: 'long',
            day: 'numeric',
            year: 'numeric',
          })}
